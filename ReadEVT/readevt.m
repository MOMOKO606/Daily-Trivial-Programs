...
for sation=1:evt.stn_sum;
 ddd=whead(sation).name;
 stationname=char(ddd);
 stationname=stationname';
 stationname=deblank(stationname);
 ...
 fprintf(fout,'%f  %f  %f\n',whead(sation).longitude1,whead(sation).latitude1,whead(sation).altitude1);    
   ...
   if length(whead(sation).seismometer)~=0           
    ...
    fprintf(fout,'%s\n','gain U>E>N');%日期
    fprintf(fout,'%f   %f    %f\n',whead(sation).gain(1),whead(sation).gain(3),whead(sation).gain(2));   %%%仪器增益  UD EW NS
    %%%%%%%%days:某年某月某日为该年的第几天
    fprintf(fout,'0\n');
